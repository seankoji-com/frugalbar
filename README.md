...
| **Anthropic Claude** | OAuth usage endpoint (`GET https://api.anthropic.com/api/oauth/usage`), via CLI Proxy or directly | Live 5-hour and 7-day quota (plus the Opus/Sonnet weekly window when reported) from the active CLI Proxy or Claude Code OAuth session |
...
- **Claude quota querying**: FrugalBar reads Anthropic's OAuth usage endpoint, through the CLI Proxy management API when one is configured and directly with the Claude Code OAuth token otherwise. Neither path sends a model request, so polling does not spend the quota it reports.
